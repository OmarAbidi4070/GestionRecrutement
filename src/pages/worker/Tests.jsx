    ...
    const fetchAssignedTest = async () => {
      try {
        ...
        if (err.response && err.response.status === 404) {
          // Pas de test assigné, ce n'est pas une erreur
          setLoading(false)
        } else {
        ...
        {!testAssignment ? (
          ...
          <div className="card">
            ...
            <div className="test-result">
              ...
              <p>
                Vous avez répondu correctement à <strong>{testResult.correctAnswers}</strong> questions sur{" "}
                <strong>{testResult.totalQuestions}</strong>.
              </p>
            </div>
          </div>
        ) : !testStarted ? (
